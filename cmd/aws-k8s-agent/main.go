...
func _main() int {
	...
	enabledVPCFlowlogEnrichment := utils.GetBoolAsStringEnvVar("VPC_FLOWLOG_ENRICHMENT", false)
	withApiServer := false
	// Check API Server Connectivity
	if enabledPodEni || enabledCustomNetwork || enabledPodAnnotation || enabledVPCFlowlogEnrichment {
		log.Info("SGP, custom networking, pod annotation, or VPC flowlog enrichment feature is in use, waiting for API server connectivity to start IPAMD")
